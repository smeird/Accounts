...
$classificationOptions = Tag::searchOptions('Merge source', 10, true);
assertEqual($tagLifecycleCategory, (int)($classificationOptions[0]['category_id'] ?? 0), 'Classification-aware tag search returns the linked category');
assertEqual($tagLifecycleSegment, (int)($classificationOptions[0]['segment_id'] ?? 0), 'Classification-aware tag search returns the category segment');
...
// Blank optional number inputs must persist as NULL under PostgreSQL strict typing.
...
$postgresStatements = implode(";\n", SchemaCatalog::createStatements());
assertEqual(true, PHP_VERSION_ID >= 80500, 'Runtime uses PHP 8.5 or newer');
assertEqual(false, (bool)preg_match('/AUTO_INCREMENT|TINYINT|LONGTEXT|ENUM\s*\(|ENGINE=|`/i', $postgresStatements), 'Canonical schema emits PostgreSQL-native DDL');
assertEqual(true, strpos($postgresStatements, 'GENERATED BY DEFAULT AS IDENTITY') !== false, 'Canonical schema uses PostgreSQL identity columns');
assertEqual(true, strpos($postgresStatements, 'CREATE INDEX IF NOT EXISTS') !== false, 'Canonical schema emits PostgreSQL indexes separately');
$translatedDateSql = ApplicationPDO::normaliseSql('SELECT YEAR(`date`), MONTH(`date`) FROM `transactions`', 'pgsql');
assertEqual('SELECT EXTRACT(YEAR FROM "date"), EXTRACT(MONTH FROM "date") FROM "transactions"', $translatedDateSql, 'PostgreSQL adapter translates legacy date helpers and identifier quoting');
$runtimePhpFiles = array_filter(
    iterator_to_array(new RecursiveIteratorIterator(new RecursiveDirectoryIterator(__DIR__ . '/../php_backend'))),
    static fn (SplFileInfo $file): bool => $file->isFile()
        && $file->getExtension() === 'php'
        && !str_contains($file->getPathname(), DIRECTORY_SEPARATOR . 'migrations' . DIRECTORY_SEPARATOR)
);
$runtimePhpSource = implode("\n", array_map(static fn (SplFileInfo $file): string => (string)file_get_contents($file->getPathname()), $runtimePhpFiles));
assertEqual(false, (bool)preg_match('/PDO::ATTR_DRIVER_NAME\)\s*===\s*[\'\"]mysql[\'\"]|FOREIGN_KEY_CHECKS|INNODB/i', $runtimePhpSource), 'Runnable PHP has no MySQL-only driver branches or session commands');
...
assertEqual(true, $equivalentSchemaAudit['healthy'], 'Database Health accepts equivalent PostgreSQL foreign-key actions and index names');

...
$nativeDateSql = ApplicationPDO::normaliseSql('SELECT EXTRACT(YEAR FROM "date"), EXTRACT(MONTH FROM "date") FROM "transactions"', 'pgsql');
assertEqual('SELECT EXTRACT(YEAR FROM "date"), EXTRACT(MONTH FROM "date") FROM "transactions"', $nativeDateSql, 'PostgreSQL adapter preserves native SQL');

...
const init = () => {
  ...
  const helpTexts = {
    ...
    'application_updates.html': `Check whether this installation is behind its matching GitHub branch and install a safe fast-forward update without using SSH. Updates stop if local files have changed or branch history needs manual review.`,
    ...
    'group_analysis.html': `Choose a group to see its spending by tag, such as taxis or hotels. Leave dates blank for full history or enter an inclusive date range. Money received is shown separately and net cost means spending less money received. Transfers and IGNORE entries are excluded from totals; the all-entries link includes them. Select any amount to inspect its transactions.`,

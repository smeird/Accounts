...
const init = () => {
  ...
  const helpTexts = {
    ...
    'all_years_dashboard.html': `The long-term view now opens in Financial Trends, where you can compare all recorded years and switch between categories, segments, groups and tags.`,
    ...
    'group_dashboard.html': `Group analysis now opens in Financial Trends, with fair period comparisons and direct links to the underlying transactions.`,
    ...
    'monthly_dashboard.html': `Monthly analysis now opens in Financial Trends, where partial months are labelled and compared using the same elapsed dates.`,
    ...
    'processes.html': `Run maintenance tasks such as auto‑tagging, category or segment assignment to tidy your data. Choose a process from the list, start it and watch the progress indicator until it completes. These tools save time by doing repetitive work for you, leaving you free to focus on decisions rather than data entry.`,
    ...
    'ai_tags.html': `Use AI to automatically suggest tags and assign categories for untagged transactions. Enter your OpenAI API token then run the tool to label multiple transactions at once and track token usage.`,
